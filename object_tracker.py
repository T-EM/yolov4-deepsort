...
flags.DEFINE_string('weights', './checkpoints/yolov4-416-tiny.tflite',
                    'path to weights file')
flags.DEFINE_integer('size', 416, 'resize images to')
flags.DEFINE_boolean('tiny', True, 'yolo or yolo-tiny')
flags.DEFINE_string('model', 'yolov4', 'yolov3 or yolov4')
flags.DEFINE_string('video', "2", 'path to input video or set to 0 for webcam')
    ...
    while True:
        ...
        if count > 0:
            ...
            for track in tracker.tracks:
                ...
                if int(track.track_id) not in name_dic:
                    name_dic[int(track.track_id)] = str(track.track_id)

                try:
                    if int(track.track_id) == int(name_dic[int(track.track_id)]):
                        ############ Face Detection ############
                        try:
                            frm = frame.copy()
                            frm = frm[int(bbox[1]):int(bbox[3]),
                                      int(bbox[0]):int(bbox[2])]
                            gray = cv2.cvtColor(frm, cv2.COLOR_BGR2GRAY)
                            faces = face_cascade.detectMultiScale(
                                gray, scaleFactor=1.5, minNeighbors=1)  # Recog. faces
                            for (x, y, w, h) in faces:
                                # Convert Face to greyscale
                                roi_gray = gray[y:y+h, x:x+w]

                                id_, conf = recognizer.predict(
                                    roi_gray)  # recognize the Face
                                if conf >= 50:
                                    name_dic[int(track.track_id)] = str(
                                        labels[id_])
                ...
                cv2.putText(frame, str(name_dic[int(track.track_id)]) + "-" + str(track.track_id),
                            (int(bbox[0]), int(bbox[1]-10)), 0, 0.75, (255, 255, 255), 2)
